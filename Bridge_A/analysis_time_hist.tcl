...
set gmDataFile_L [open "$gm_file_loc/gm_1.txt" "r"];
set gmDataFile_T [open "$gm_file_loc/gm_2.txt" "r"];
set gmDataFile_V [open "$gm_file_loc/gm_3.txt" "r"];
...
if {$ok == 0} {
	...
	while {$ok == 0 && $tCurrent <= $totalAnalysisTime} {
		...
		if {$ok != 0} {
			# puts $analysisLogFile "Try NewtonLineSearch with Unbalance"
			test NormUnbalance $tolDynUnb $maxNumIterDyn $showTest;
...
if {$ok == 0} {
    set tCurrent [getTime];
    if {[expr $tCurrent/$totalAnalysisTime] >= 0.85} {
        puts "NLTHA COMPLETE!"
        set NLTHA_OK 1;
    } else {
        puts "NLTHA FAILED!"
