...
set Avf [expr 0.];
set As1 [expr 12.*$Ab_25];
set As2 [expr 4.*$Ab_22];
set Ass1 [expr $Ab_16];
set Ass2 [expr $Ab_19];
...
for {set i 1} {$i <= 4} {incr i 1} {
    ...
    set Vn [expr $Vc + $Vs];
    set Ld [expr (($db_25/$in)*($fy/$psi)/(25.*sqrt($fpc/$psi)))*$in];
    set D2 [expr sqrt(2.)*$ey*($Ld + $La)*($h_SK + $d1_SK)/sqrt(pow($h_SK,2) + pow($d_SK,2))];
    set Ld [expr (($db_16/$in)*($fy/$psi)/(25.*sqrt($fpc/$psi)))*$in];
...
if {$bf_skew == 1} {
set fac {0.096201560151973 0.126748015076754 0.167136960854757 0.221823882184715 0.388089581731801}
} else {
...
if {$write_model_files == 1} {
	puts $matDataFileID "uniaxialMaterial Elastic 	  20000 	[expr 1.e-10];"
	puts $matDataFileID "uniaxialMaterial Elastic       203       [expr 1.e6];"
}
#
# ################################################################################################################################################################################################################################################################
# ################################################################################################################################################################################################################################################################
...
puts $ExportID "element zeroLength 1501 1501 1500 -mat 8001 -dir 2 -doRayleigh $doRayleighSK -orient  1 0 0 0 1 0;"
puts $ExportID "element zeroLength 1601 1601 1600 -mat 8002 -dir 2 -doRayleigh $doRayleighSK -orient -1 0 0 0 -1 0;"
puts $ExportID "element zeroLength [expr ($nSpans + 1)*1000 + 500 + 1] [expr ($nSpans + 1)*1000 + 500 + 1] [expr ($nSpans + 1)*1000 + 500 + 0] -mat 8003 -dir 2 -doRayleigh $doRayleighSK -orient 1 0 0 0 1 0;"
puts $ExportID "element zeroLength [expr ($nSpans + 1)*1000 + 600 + 1] [expr ($nSpans + 1)*1000 + 600 + 1] [expr ($nSpans + 1)*1000 + 600 + 0] -mat 8004 -dir 2 -doRayleigh $doRayleighSK -orient -1 0 0 0 -1 0;"
}
