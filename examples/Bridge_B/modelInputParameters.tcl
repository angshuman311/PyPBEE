...
# Deck
# ----------------------
set L1				[expr 154.82*$ft];								# Length of span 1
set L2				[expr 144.98*$ft];								# Length of span 2
...
set dw				[expr 75.5*$ft];								# Deck Width
set dd				[expr 6.23*$ft];								# Deck Depth
set Adeck			[expr 129.13*pow($ft,2)];						# Area of deck
set Jdeck			[expr 2532.*pow($ft,4)];						# Torsional constant of deck
set Iydeck			[expr 791.76*pow($ft,4)];						# Iy - local
set Izdeck			[expr 58352.*pow($ft,4)];						# Iz - local
set gapL			[expr 1.0*$in];									# Gap between the backwall and deck

# Cap Beam
# ----------------------
set nCapBeamEle 1;
set Lcap1L			[expr 18.04*$ft];										# Cap Beam Length
set Lcap1R			[expr 18.04*$ft];										# Cap Beam Length
set Lcap			{CapBeamLengths $Lcap1R $Lcap1L};					# Cap Beam Lengths List {Bent(Right, Left), ..}
set bcap1  			[expr 7.55*$ft];										# Cap Beam Breadth
set hcap1  			[expr 6.23*$ft];										# Cap Beam Height
...
# Columns
# ----------------------
set nCols			2;												# number of columns per bent
set dcg				[expr 3.41*$ft];								# Deck Centeriod-Column Top Dist.
...
set sH1					[expr 3.5433*$in];												# spacing of hoops
...
set HColumnMax		[expr 22.0*$ft];								# Column Height
...
set DbarBH1			[expr 1.4066*$in];                 				# using #35 reinforcing bars
set numBarBH1		12;												# Number of rebar per column BH
...
set DBH				{BaseHingeDiameters $DBH1};						# Column BH Diameter
set DbarBH			{BaseHingeReinfDia $DbarBH1};					# using #35 reinforcing bars
set numBarBH		{BaseHingeReinfNumberofBars $numBarBH1};		# Number of rebar per column BH
set coverBH			{BaseHingeCover $coverBH1};
set ABH				{BaseHingeAreas $ABH1};							# Area of column BH
set JBH				{BaseHingeJ $JBH1};								# Torsional constant of column BH
set IzBH			{BaseHingeIz $IzBH1};							# Iz - local BH
set IyBH			{BaseHingeIy $IyBH1};							# Iy - local BH
set bundlesBH		{BaseHingeReinfBundle $bundleBH1};               # Bundles of reinforcing bar to be placed at each fiber location
...
# SSI
# ----------------------
set kx_ssi [expr 1565. * $MN / $m]
set ky_ssi [expr 1613. * $MN / $m]
set kz_ssi [expr 1369. * $MN / $m]
set mx_ssi [expr 11020.* $MN * $m / 1.]
set my_ssi [expr 16732.* $MN * $m / 1.]
set mz_ssi [expr 23339.* $MN * $m / 1.]

set cx_ssi [expr 26. * $MN * $sec / $m]
set cy_ssi [expr 25. * $MN * $sec / $m]
set cz_ssi [expr 28. * $MN * $sec / $m]
set cmx_ssi [expr 61. * $MN * $m * $sec / 1.]
set cmy_ssi [expr 88. * $MN * $m * $sec / 1.]
set cmz_ssi [expr 85. * $MN * $m * $sec / 1.]

# Bearings
# ----------------------
set nBearings		7;												# number of bearing pads
set hb [expr 95.*$mm];
set hb [expr $hb/1.5];
set Gb [expr 0.69*$MPa];
set Ab [expr 600.*$mm*600.*$mm];
set kInit1 [expr ($nBearings/5.)*$Gb*$Ab/$hb];
set qd1 [expr ($nBearings/5.)*$Gb*$Ab/2.];

# Shear Keys
# --------------------------------------------------------------
set b_SK [expr 45.7*$in];
set d_SK [expr 74.4*$in];
set d1_SK [expr 69.3*$in];
set h_SK [expr 77.14*$in];
set sh_SK [expr 7.88*$in];
set sv_SK [expr 7.88*$in];
set s_SK $sh_SK;
...
set db_15 [expr 0.6304*$in];
set Ab_15 [expr $pi*pow($db_15,2)/4.];

set db_20 [expr 0.7683*$in];
set Ab_20 [expr $pi*pow($db_20,2)/4.];

set db_35 [expr 1.4066*$in];
set Ab_35 [expr $pi*pow($db_35,2)/4.];
