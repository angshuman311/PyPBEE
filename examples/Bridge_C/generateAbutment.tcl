...
set As1 [expr 6.*2.*$Ab_36];
set As2 [expr 5.*$Ab_22];
set Ass1 [expr $Ab_16];
set Ass2 [expr $Ab_36];
set Ass3 [expr $Ab_19];
...
for {set i 1} {$i <= 4} {incr i 1} {
    ...
    set ey [expr $fy/$Es];
    set Vs_in [expr ($As1*$fy*$h_SK + $As2*$fy*$d_SK + $nh*$Ass1*$fy*pow($h_SK,2)/(2.*$sh_SK) + $Ass3*$fy*pow($h_SK,2)/(2.*$sh_SK_back) + $Ass2*$fy*pow($d_SK,2)/(2.*$sv_SK) + $nv*$Ass1*$fy*pow($d_SK,2)/(2.*$sv_SK))*(1/($h_SK + $a_SK))];
    set Vs_out [expr (16.*$Ab_22*$fy*$d_out_cracked_SK)*(1/($h_SK + $a_SK))];
    set Vc_in [expr 2.4*sqrt($fpc/$psi)*($b_SK/$in)*($h_SK/$in)*$lbf];
    set Vc_out [expr 2.4*sqrt($fpc/$psi)*($d_SK/$in)*($h_SK/$in)*$lbf];
    set kai [expr exp(-$skew/40.)];
    set Vc [expr $kai*$Vc_in + (1 - $kai)*$Vc_out];
    set Vs [expr $kai*$Vs_in + (1 - $kai)*$Vs_out];
    set Vn [expr $Vc + $Vs];
    set Ld [expr (($db_36/$in)*($fy/$psi)/(25.*sqrt($fpc/$psi)))*$in];
...
if {$bf_skew == 1} {
set fac {0.092758409631002 0.132903073113485 0.180612614189819 0.247805050091646 0.345920852974048}
} else {
